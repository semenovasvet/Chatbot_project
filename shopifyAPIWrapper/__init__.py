...
from .shopify_operations.deliveryProfile import DeliveryProfile
from . import config
from .config import Metafield_Key, Payment_Terms, Email_Subscription, Catalog_Status, PriceList_Adjustment_Type, PriceList_CompareAtMode, Opt_In_Level
...
class Shopify:
    ...
    def customer_create(self, email: str,
                            ...
                            marketingOptInLevel: str = Opt_In_Level.SINGLE_OPT_IN.value,
                            tags: list = []) -> dict:
        ...
        response = self.customer.customer_create(email=email, address=address, city=city, zipcode=zipcode, countryCode=countryCode, firstName=firstName, lastName=lastName, phone=phone, company=company, emailSubscription=emailSubscription, marketingOptInLevel=marketingOptInLevel, tags=tags)
    ...
    def customer_emailMarketingConsentUpdate(self, customerId: str, marketingOptInLevel: str = Opt_In_Level.SINGLE_OPT_IN.value, emailSubscription: str = Email_Subscription.SUBSCRIBED.value) -> dict:
        """update customer's email marketing consent in shopify -> {"success": bool, "errors": Optional[dict, list, str]}"""
        if not all([customerId, marketingOptInLevel, emailSubscription]):
            raise ValueError(f"Some of the values might be absent: {customerId=}, {marketingOptInLevel=}, {emailSubscription=}")
        ...
        if not any(member.value == marketingOptInLevel for member in Opt_In_Level):
            raise ValueError(f'marketingOptInLevel must be one of {[member.value for member in Opt_In_Level]}')
        if not any(member.value == emailSubscription for member in Email_Subscription):
            raise ValueError(f'emailSubscription must be one of {[member.value for member in Email_Subscription]}')
        
        action = config.CUSTOMER_EMAIL_MARKETING_CONSENT_UPDATE
        response = self.customer.customer_marketing_consent_update(customerId, marketingOptInLevel=marketingOptInLevel, emailSubscription=emailSubscription)
        if response and action in response and response[action]:
            logging.info(f'Customer with id {customerId} was updated!')
            return {"success": True, "errors": response[action][config.USER_ERRORS]}
        logging.error(f'Error when trying to update customer with id {customerId}: {response}')
        return {"success": False, "errors": response}
    ...
    def deliveryProfile_update(self, variants_to_associate: list = [], variants_to_dissociate: list = [], delivery_profile_id: str = config.SHIPMENT_7_12_DAYS) -> dict:
        """update delivery profile"""

        if not variants_to_associate and not variants_to_dissociate:
            raise ValueError("Both variants_to_associate and variants_to_dissociate are empty. Can not update delivery profile.")
        if config.SHOPIFY_ID_DELIVERY_PROFILE not in delivery_profile_id:
            raise ValueError(f'delivery_profile_id must be in format {config.SHOPIFY_ID_DELIVERY_PROFILE}')
        if not isinstance(variants_to_associate, list) or not isinstance(variants_to_dissociate, list):
            raise TypeError("variants_to_associate and variants_to_dissociate must both be lists.")

        action = config.DELIVERY_PROFILE_UPDATE
        response = self.deliveryProfile.deliveryProfile_update(variants_to_associate, variants_to_dissociate, delivery_profile_id)
        if response and action in response and response[action]:
            logging.info(f'DeliveryProfile was successfully updated')
            return {"success": True, "errors": response[action][config.USER_ERRORS]}
        logging.error(f'Error when trying to update deliveryProfile: {response}')
        return {"success": False, "errors": response}